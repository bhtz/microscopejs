...
(function(){
	...
	*/
    ...
    ViewScaffolder.prototype.renderInputByProperty = function (modelname, property) {
        var input;
        var value = '<%= ' + modelname + '.' + property.name + ' == undefined ? "" : ' + modelname + '.' + property.name + ' %>';
        switch (property.type) {
            case 'STRING':
                input = '<input type="text" id="' + property.name + '" name="' + property.name + '" value="'+value+'"/>\n';
                break;
            case 'PASSWORD':
                input = '<input type="password" id="' + property.name + '" name="' + property.name + '" value="'+value+'"/>\n';
                break;
            case 'TEXT':
                input = '<textarea id="' + property.name + '" name="' + property.name + '">'+value+'</textarea>\n';
                break;
            case 'INTEGER':
                input = '<input type="number" id="' + property.name + '" name="' + property.name + '" value="'+value+'"/>\n';
                break;
            case 'DATE':
                input = '<input type="date" id="' + property.name + '" name="' + property.name + '" value="'+value+'"/>\n';
                break;
            case 'BOOLEAN':
                input = '<input type="checkbox" id="' + property.name + '" name="' + property.name + '" checked="'+value+'" />\n';
                break;
            case 'FLOAT':
                input = '<input type="number" id="' + property.name + '" name="' + property.name + '" value="'+value+'"/>\n';
                break;
