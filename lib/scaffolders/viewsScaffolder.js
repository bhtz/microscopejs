	...
	*/
    ...
    ViewScaffolder.prototype.renderInputByProperty = function (modelname, property) {
        ...
        switch (property.type) {
            case 'STRING':
                input.html = '<%- f.text_field("'+ property.name +'", {class: "form-control"}) %>';
                break;
            case 'PASSWORD':
                input.html = '<%-f.password_field("'+property.name+'", {class: "form-control"}) %>';
                break;
            case 'TEXT':
                input.html = '<%- f.text_area("'+property.name+'", {class: "form-control"}) %>';
                break;
            case 'INTEGER':
                input.html = '<%- f.number_field("'+property.name+'", {class: "form-control"}) %>';
                break;
            case 'DATE':
                input.html = '<%- f.date("'+property.name+'", {class: "form-control"}) %>';
                break;
            case 'BOOLEAN':
                input.html = '<%- f.select("'+property.name+'",[{"value":0,"text":"false"},{"value":1,"text":"true"}], {class: "form-control"}) %>';
                break;
            case 'FLOAT':
                input.html = '<%- f.number_field("'+property.name+'", {class: "form-control"}) %>';
                break;
